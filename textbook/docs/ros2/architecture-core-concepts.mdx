...
Welcome to the Robot Operating System 2 (ROS 2)—the middleware that serves as the nervous system of modern robots. In this chapter, you'll learn how ROS 2 enables different parts of a robot to communicate, coordinate, and work together as a unified system.

:::info What You'll Build
By the end of this chapter, you'll create your first ROS 2 node that publishes sensor data and subscribes to control commands—the foundation of every robot system.
:::

:::tip Why This Matters
ROS 2 is used by Boston Dynamics, NASA, Amazon Robotics, and thousands of robotics companies. Understanding ROS 2 is essential for any robotics career.
:::
...
### ROS 1 vs ROS 2: Complete Comparison

| Feature | ROS 1 | ROS 2 |
|---------|-------|-------|
| **Architecture** | Centralized (roscore required) | Distributed (no master needed) |
| **Communication** | Custom TCP/UDP | DDS (Data Distribution Service) |
| **Real-time** | Not supported | Supported with proper configuration |
| **Security** | None built-in | SROS2 (encryption, authentication) |
| **Platforms** | Linux only | Linux, Windows, macOS, RTOS |
| **Lifecycle** | None | Managed node lifecycle states |
| **QoS** | Limited (TCPROS/UDPROS) | Full QoS policies (reliability, durability) |
| **Discovery** | Master-based | Automatic peer discovery |
...
| **Status** | Maintenance only (EOL 2025) | Active development |

<div className="key-takeaway">

**Why ROS 2?** The shift from ROS 1 to ROS 2 isn't just an upgrade—it's a fundamental redesign for production robotics. ROS 2's distributed architecture, real-time capabilities, and built-in security make it suitable for commercial deployment, not just research.

</div>

:::tip Migration Note
If you're coming from ROS 1, the biggest conceptual change is the removal of the master node. In ROS 2, nodes discover each other automatically through DDS, making your system more resilient to single points of failure.
:::
...
#### Exercise: Create Your First ROS 2 Publisher-Subscriber System

In this exercise, you'll create a simple publisher that sends messages and a subscriber that receives them.

**Step 1:** Create a new Python file called `simple_publisher.py`:
```python
import rclpy
from rclpy.node import Node
...
**Step 3:** In another terminal, echo the topic:
```bash
...
<span className="question-number">Q1.</span> Which ROS 2 communication pattern should you use for streaming sensor data at 30 Hz?
...
<div className="question">
<span className="question-number">Q2.</span> What is the main advantage of using Actions over Services?
<div className="options">

- A) Actions are faster
- B) Actions provide feedback during execution and can be canceled ✓
- C) Actions use less memory
- D) Actions don't require a server

</div>
</div>

<div className="question">
<span className="question-number">Q3.</span> Complete the code to create a publisher:
```python
self.publisher = self.create_publisher(
    String,
    '/my_topic',
...
<a href="/ros2/python-integration" className="next-chapter">
  Next: Python ROS 2 Integration →
</a>
