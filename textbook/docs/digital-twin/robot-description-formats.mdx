...
### URDF vs SDF: Complete Comparison

| Feature | URDF | SDF |
|---------|------|-----|
| **Primary Use** | ROS robot description | Gazebo simulation |
| **Scope** | Single robot only | Worlds + multiple robots + lights |
| **Kinematic Loops** | Not supported (tree only) | Fully supported |
| **Sensors** | Basic (via Gazebo tags) | Comprehensive native support |
| **Physics Config** | Limited | Full engine configuration |
| **File Extension** | `.urdf`, `.xacro` | `.sdf`, `.world` |
| **Macros/Templates** | Via Xacro | Via ERB or custom tools |
| **Surface Properties** | Basic friction | Full contact/friction/bounce |
| **Lighting** | Not supported | Full light definitions |
| **Plugins** | Via `<gazebo>` tags | Native plugin support |
| **Model Composition** | Include via Xacro | Native `<include>` tag |
| **Pose Specification** | `<origin xyz rpy>` | `<pose>x y z roll pitch yaw</pose>` |
| **ROS Integration** | Native | Via ros_gz_bridge |

<div className="key-takeaway">

**Best Practice:** Use URDF with Xacro for your robot model (ensures ROS compatibility with robot_state_publisher, MoveIt, etc.), then add Gazebo-specific features using `<gazebo>` tags. Gazebo automatically converts URDF to SDF internally.

</div>

:::tip When to Use Each Format
- **URDF**: When you need ROS tools (TF, MoveIt, RViz visualization)
- **SDF**: When building simulation worlds, adding multiple robots, or need advanced physics
- **Both**: Most projects use URDF for the robot + SDF for the world environment
:::
...
#### Exercise: Create a Simple URDF Robot Arm

In this exercise, you'll create a basic 2-link robot arm URDF from scratch.

**Step 1:** Create a file called `simple_arm.urdf`:
```xml
<?xml version="1.0"?>
